...
Mesh::Mesh(Vertex* vertices, int numVertices, unsigned int* indices, int numIndices)
{
	...
	vertexBuffer = DX12Helper::GetInstance().CreateStaticBuffer(sizeof(Vertex), numVertices, vertices);
	indexBuffer = DX12Helper::GetInstance().CreateStaticBuffer(sizeof(unsigned int), numIndices, indices);
}
...
// File input object
	...
	vertexBuffer = DX12Helper::GetInstance().CreateStaticBuffer(sizeof(Vertex), numVertices, &verts[0]);
	indexBuffer = DX12Helper::GetInstance().CreateStaticBuffer(sizeof(unsigned int), numIndices, &indices[0]);
}
